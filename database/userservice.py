...
def user_answer_db(user_answer, user_id, q_id, level):
    with next(get_db()) as db:
        ...
        if exact_question.correct_answer == user_answer:
            ...
            db.add(new_answer)
            db.commit()
            user_rating = db.query(Rating).filter_by(user_id=user_id, level=level).one()
            if user_rating:
                user_rating.correct_answers += 1
                db.commit()
            elif not user_rating:
                new_rating = Rating(user_id=user_id,
                                    level=level, correct_answer=1)
                db.add(new_rating)
                db.commit()
            return True
        else:
            wrong_answer = UserAnswer(user_id=user_id, question_id=q_id,
